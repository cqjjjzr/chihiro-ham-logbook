    ...
    <div class="${(this.detailsDialogOpen ? 'block' : 'hidden')} fixed z-10 left-0 top-0 w-full h-full overflow-auto bg-black/60 transition-opacity" @click="${() => { this.detailsDialogOpen = false; }}">
        <div class="mx-auto mt-8 w-[60em] bg-white p-8 border-slate-900"
          ...
          <div class="flex flex-col mt-3">
            ...
            ${this.detailsDialogContent?.gridsquare ? html`<div class="detail-row"><span class="w-[30%]">${msg('对方网格坐标')}</span><span>${this.detailsDialogContent.gridsquare}</span></div>` : null}
            ${this.detailsDialogContent?.propagationMode ? html`<div class="detail-row"><span class="w-[30%]">${msg('传播方式')}</span><span>${this.detailsDialogContent.propagationMode}</span></div>` : null}
            ${this.detailsDialogContent?.satellite ? html`<div class="detail-row"><span class="w-[30%]">${msg('卫星')}</span><span>${this.detailsDialogContent.satellite}</span></div>` : null}
          </div>
        </div>
      </div>
    `;
      ...
      try {
        ...
        this.data = logdata.records?.map((record: { [name: string]: string }) => {
          return {
            ...
            gridsquare: record['gridsquare']?.trim(),
            selfGridsquare: record['my_gridsquare']?.trim(),
            selfRig: record['my_rig_intl']?.trim(),
            propagationMode: record['prop_mode']?.trim(),
            satellite: record['sat_name']?.trim()
          } as LogbookEntry;
